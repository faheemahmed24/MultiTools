...
const PureOrganizer: React.FC<{ t: TranslationSet }> = ({ t }) => {
  ...
  const handleCopyAll = () => {
    if (!result) return;
    const textToCopy = result.categories.map((cat: any) => {
      return `${cat.heading.toUpperCase()}\n${cat.items.join('\n')}`;
    }).join('\n\n');
  ...
  return (
    <div className="flex flex-col h-full animate-fadeIn max-w-7xl mx-auto w-full gap-8">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b border-white/5 pb-8">
        ...
        {result && (
          <div className="flex flex-wrap gap-3 animate-pop-in justify-center">
            <button onClick={handleCopyAll} className="group flex items-center gap-3 bg-purple-600/10 hover:bg-purple-600/20 text-purple-400 px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest border border-purple-500/20 transition-all">
