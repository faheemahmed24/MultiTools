...
const VideoToAudio: React.FC<{ t: TranslationSet, onConversionComplete: (data: { fileName: string, outputFormat: string }) => void }> = ({ t, onConversionComplete }) => {
  ...
  const formatCategories: FormatCategory[] = [
    {
      title: t.catUncompressed || 'Uncompressed',
      formats: [
    ...
    {
      title: t.catLossless || 'Lossless Compressed',
      formats: [
        { id: 'flac', label: 'FLAC', ext: 'flac', mime: 'audio/flac', quality: 'lossless', description: 'Open Lossless' },
      ]
    },
    {
      title: t.catLossy || 'Lossy Compressed',
      formats: [
        { id: 'mp3', label: 'MP3 (MPEG)', ext: 'mp3', mime: 'audio/mpeg', quality: 'compressed', description: 'Highly Compatible' },
        { id: 'aac', label: 'AAC', ext: 'aac', mime: 'audio/aac', quality: 'compressed', description: 'Advanced Coding' },
        { id: 'ogg', label: 'OGG Vorbis', ext: 'ogg', mime: 'audio/ogg', quality: 'compressed', description: 'Xiph Open' },
        { id: 'webm', label: 'WebM Audio', ext: 'webm', mime: 'audio/webm', quality: 'compressed', description: 'Modern Web' },
      ]
    }
  ];
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const audioBuffer = await audioContext.decodeAudioData(arrayBuffer).catch(err => {
        throw new Error(`Neural Decoder Error: ${err.message || 'Incompatible audio stream format'}`);
      });

      const target = formatCategories.flatMap(c => c.formats).find(f => f.id === outputFormat);

      // Simple browser-side logic:
      // WAV/AIFF can be manually encoded. 
      // For others, use MediaRecorder if supported, or fallback to WAV with renamed extension.
      if (['wav', 'aiff'].includes(outputFormat)) {
        ...
        setProgress('Format not natively encodable, using high-fidelity master...');
        await convertToWav(audioBuffer);
      }
    } catch (err) {
      console.error(err);
  ...
  const allFormats = formatCategories.flatMap(c => c.formats);
  const currentFmt = allFormats.find(f => f.id === outputFormat);
                         ...
                         <div className="absolute bottom-full mb-3 right-0 w-48 bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl py-2 z-50 animate-pop-in" onMouseLeave={() => setShowFormatsMenu(false)}>
                            ...
                            {allFormats.map(fmt => (
                                <a
                                    key={fmt.id}
                                    href={audioUrl}
                                    download={`${videoFile.name.replace(/\.[^/.]+$/, "")}.${fmt.ext}`}
