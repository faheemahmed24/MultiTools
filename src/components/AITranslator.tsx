...
import { ArrowLeftRight, Copy, Check, Download, XCircle } from 'lucide-react';
...
const AITranslator: React.FC<AITranslatorProps> = ({ t, onTranslationComplete }) => {
  ...
  const handleSwapLanguages = () => {
    ...
    const newSourceInTargetList = targetLanguages.find((l: any) => l.code === currentTarget.code);
    const newTargetInSourceList = sourceLanguages.find((l: any) => l.code === currentSource.code);
  ...
  return (
    <div className="bg-gray-800 rounded-2xl shadow-lg p-6">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-4">
        ...
        >
          <ArrowLeftRight className={`w-6 h-6 text-gray-300 transition-transform duration-500 ${isSwapping ? 'rotate-[360deg]' : ''}`} />
        </button>
        <LanguageDropdown
