...
const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <>
      ...
      <aside 
          className={`bg-[#05050C] border-r border-white/5 flex flex-col transition-all duration-500 ease-in-out fixed md:relative inset-y-0 left-0 z-[70] md:z-50 shrink-0 group/sidebar ${isSidebarOpen ? 'w-[280px] translate-x-0' : 'w-[80px] -translate-x-full md:translate-x-0'}`}
          onMouseEnter={() => { if (window.innerWidth >= 768) setIsSidebarOpen(true); }}
          onMouseLeave={() => { if (window.innerWidth >= 768) setIsSidebarOpen(false); }}
      >
        <div className="flex items-center justify-between h-24 px-5 mb-2 overflow-hidden select-none">
          <div className="flex items-center gap-4 min-w-[240px] cursor-pointer" onClick={() => { setActiveTool('Home'); if (window.innerWidth < 768) setIsSidebarOpen(false); }}>
        ...
        <nav className="flex-grow overflow-y-auto px-4 space-y-4 no-scrollbar pb-10">
          ...
          <button onClick={() => { setActiveTool('History'); setActiveHistoryTab('transcriptions'); if (window.innerWidth < 768) setIsSidebarOpen(false); }} className={`w-full flex items-center gap-4 p-3.5 rounded-2xl transition-all relative ${activeTool === 'History' ? 'bg-pink-600/10 text-white' : 'text-gray-500 hover:bg-white/5'}`}>
